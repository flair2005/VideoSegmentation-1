...
% dir_path = '/home/yangzheng/testData/ucsd/vidf1_33_006.y';
...
D = 2;                                % positive deviation threshold比较门槛值
alpha = 0.1;                           % learning rate (between 0 and 1) (from paper 0.01)初始化的学习率
        ...
        for w=1:width
            ...
            for c=1:C
                if (abs(u_diff(h,w,c,1)) <= D*sd(h,w,c,1))       % pixel matches component
                    ...
                    % update weightmatch(1) = 1s, mean, sd, p 更新每个像素的高斯模型，如果当前像素与某个背景高斯模型相匹配，则对背景进行更新
            ...
            bg_bw(h,w)=0;
            for c=1:C
                bg_bw(h,w) = bg_bw(h,w)+ (mean(h,w,c,1)*weight(h,w,c,1)*2/3+mean(h,w,c,2)*weight(h,w,c,2)*1/3);%更新背景
%                 bg_bw(h,w) = bg_bw(h,w)+ (mean(h,w,c,1)*weight(h,w,c,1));
                %%5............
                %if(bg_bw(i,j)>thresh)
                % k=k-1;
                %  M=k;
            ...
            if (match(1) == 0)
                ...
                mean(h,w,min_w_index,1) = double(X(h,w,1));%fr_bw为背景灰度图像，由于没有匹配，所以保留原值不变
                %我认为是当前像素值作为新模型的均值
            ...
            if (match(2) == 0)
                ...
                %找出w中的所有维数(也就是所有高斯模型)对应的第i行，第j列的最小值（也就是权重），
                %返回值min_w为数值（权重）大小，min_w_index为对应的维数（高斯模型）
                mean(h,w,min_w_index,2) = double(X(h,w,2));%fr_bw为背景灰度图像，由于没有匹配，所以保留原值不变
                %我认为是当前像素值作为新模型的均值
                sd(h,w,min_w_index,2) = sd_init;%没有匹配，保留原值不变
            end
            
            
            rank = (weight(h,w,:,1)./sd(h,w,:,1)*2/3+weight(h,w,:,2)./sd(h,w,:,2)*1/3);             % calculate component rank
...
%             for c=1:C
%                 for dg=1:D_Gauss
%                     u_diff(:,:,c,dg) = abs(X(:,:,dg) - double(mean(:,:,c,dg)));
%                 end
%             end
            % 跟新前景
            fg(h,w) = 0; %前景
            while ((match(1) == 0)&&(k<=M))% 没有匹配，而且当前高斯数号小于背景数  
                if (weight(h,w,rank_ind(k)) >= thresh)
                    if (abs(u_diff(h,w,rank_ind(k),1))*2/3+abs(u_diff(h,w,rank_ind(k),2))*1/3 <= D*(sd(h,w,rank_ind(k),1)*2/3+sd(h,w,rank_ind(k),2)*1/3))
                        fg(h,w) = 255; %判断是否是背景，如果符合匹配准则就认为是背景
                        match = 1;
