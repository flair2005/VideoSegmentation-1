...
class gmm_VideoSeg_1d:
    ...
    def rank_mode_by_weight(self,h,w):
        ...
        :return: 
        """
        rank = (self.weight[h, w,:, 0] / self.sd[h, w,:, 0]) # calculate component rank
    ...
    def update_foreground(self,h,w):
        ...
        while ((match == 0) and (k < self.hyper_param.M)):
            if (self.weight[h, w, rank_ind[k],0] <= self.hyper_param.thresh):
                if (abs(self.u_diff[h, w, rank_ind[k], 0]) <= self.hyper_param.D * (
                            self.sd[h, w, rank_ind[k], 0])):
