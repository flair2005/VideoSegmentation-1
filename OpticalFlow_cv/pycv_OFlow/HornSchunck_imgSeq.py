...
if __name__ == '__main__':
    ...
    ImgSeq = {
        ...
        'fileName' : 'vidf1_33_000_f',
        'fileType' : '.png',
        'totalFrame' : 200,
        'currFrame' : 1
    }

    def readFrame(ImgSeq):
        if ImgSeq['currFrame'] >= ImgSeq['totalFrame']:
            return None

        if ImgSeq['currFrame']<10:
            strCurrFrame = '00'+str(ImgSeq['currFrame'])
        elif ImgSeq['currFrame']<100:
            strCurrFrame = '0' + str(ImgSeq['currFrame'])
        else:
            strCurrFrame = str(ImgSeq['currFrame'])

        imgPath = ImgSeq['dirPath']+'/'+ImgSeq['fileName']+strCurrFrame+ImgSeq['fileType']
        ImgSeq['currFrame'] += 1
        frame = cv2.imread(imgPath)
        return frame

    prev = readFrame(ImgSeq)  # 读取视频第一帧作为光流输入的当前帧֡
    ...
    while True:
        curr = readFrame(ImgSeq)  # 读取视频的下一帧作为光流输入的当前帧
        if curr != None:  # 判断视频是否结束
            currgray = cv2.cvtColor(curr, cv2.COLOR_BGR2GRAY)
            flow = cv2.calcOpticalFlowFarneback(prevgray, currgray, None, pyr_scale=0.5,
                    levels=3, winsize=15, iterations=3, poly_n =5,
                    poly_sigma=1.5, flags=cv2.OPTFLOW_LK_GET_MIN_EIGENVALS)  # Farnback光流法
            prevgray = currgray  # 计算完光流后，将当前帧存储为下一次计算的前一帧

            residual = me.LinearRegression(flow)
            motionflow = np.zeros((flow.shape[0], flow.shape[1]))

            for i in range(0, flow.shape[0]):
                for j in range(0, flow.shape[1]):
                    if residual[i, j] > 0.2:
                        motionflow[i, j] = math.sqrt(math.pow(flow[i,j,0], 2)+math.pow(flow[i,j,1], 2))

            cv2.imshow('flow', draw_flow(currgray, flow))
            cv2.imshow("residual", residual)
            cv2.imshow("motionflow", motionflow)
