...
CvPoint Slic::find_local_minimum(IplImage *image, CvPoint center) {
    /*min_grad临时保存最小梯度值, loc_min临时保存最小梯度像素位置*/
    double min_grad = FLT_MAX;
    CvPoint loc_min = cvPoint(center.x, center.y);

    /*遍历center为中心的3x3 patch*/
    ...
    for (int i = 0; i < image->width; i++) {
        for (int j = 0; j < image->height; j++) {
            if (new_clusters[i][j] == -1) {
                ...
                /*以(i, j)为种子点,将周围的所有联通的同类别的像素点的位置存入elements容器*/
                int count = 1;
...
void Slic::display_centers(IplImage *image, CvScalar colour){
    int i = 0;
    std::cout<<(int)centers.size()<<std::endl;
    for(;i<(int)centers.size();i++){
        if(((int)centers[i][3]>=0 && (int)centers[i][3]<=image->width)&&((int)centers[i][4]>=0 && (int)centers[i][4]<=image->height))
            cvSet2D(image, (int)centers[i][4], (int)centers[i][3], colour);
//            std::cout<<i<<" "<<(int)centers[i][3]<<" "<<(int)centers[i][4]<<std::endl;
    }
}

/*compute superpix region's feature*/
void Slic::compute_feature(IplImage *image){
    const int dx8[8] = {-1, -1,  0,  1, 1, 1, 0, -1};
	const int dy8[8] = { 0, -1, -1, -1, 0, 1, 1,  1};
    vec2dd features;
    vec2db mask;
    /// 初始化mask对象,标记某个像素时候访问过
    for(int i=0;i<image->width;i++){
        vector<bool> t_mask;
        for(int j=0;j<image->height;j++){
            t_mask.push_back(false);
        }
        mask.push_back(t_mask);
    }

    for(size_t i=0;i<centers.size();i++){
        vector<CvPoint> t_stack;
        t_stack.push_back(CvPoint(centers[i][3],centers[i][4]));
        CvScalar feature(0, 0, 0);
        for(size_t j=0;j<t_stack.size();j++){
            size_t c = 0;
            for(;c<sizeof(dx8);c++){
                int x = centers[i][3]-dx8[c];
                int y = centers[i][4]-dy8[c];
                if((x>=0&&x<image->width)&&(y>=0&&y<image->height)&&
                   (new_clusters[x][y]==new_clusters[centers[i][3]][centers[i][4]])&&
                   (mask[x][y]==false)){
                    CvScalar color = cvGet2D(image, y, x);
                    feature.val[0] += color.val[0];
                    feature.val[1] += color.val[1];
                    feature.val[2] += color.val[2];
                    mask[x][y] = true;
                    t_stack.push_back(CvPoint(x,y));
                }
            }
        }
        if(t_stack.size()>0){
            vector<double> t_feature;
            feature.val[0] /= (double)t_stack.size();
            feature.val[1] /= (double)t_stack.size();
            feature.val[2] /= (double)t_stack.size();
            t_feature.push_back(feature.val[0]);
            t_feature.push_back(feature.val[1]);
            t_feature.push_back(feature.val[1]);
            features.push_back(t_feature);
        }
    }
    vec2di similar;
    double threshold = 0.1;
    for(size_t i =0;i<centers.size();i++){
        vector<int> t_similar;
        for(size_t j=i;j<centers.size();j++){
            double dif_r = abs(features[i][0] - features[j][0])/(features[i][0] + features[j][0]);
            double dif_g = abs(features[i][1] - features[j][1])/(features[i][1] + features[j][1]);
            double dif_b = abs(features[i][2] - features[j][2])/(features[i][2] + features[j][2]);
            double diff_rgb = (dif_r+dif_g+dif_b)/3;
            if(diff_rgb<=threshold){
                t_similar.push_back((int)j);
            }
        }
        similar.push_back(t_similar);
    }
//    std::cout<<features.size()<<std::endl;
//    for(size_t i=0;i<features.size();i++){
//        std::cout<<centers[i][3]<<"  "<<centers[i][4]<<"  "<<features[i][0]<<"  "<<features[i][1]<<"  "<<features[i][2]<<std::endl;
//    }
    vector<bool> centers_mask;
    for(size_t i=0;i<centers.size();i++){
        centers_mask.push_back(false);
    }
    for(size_t i=0;i<similar.size();i++){
        vector<int> centers_idx;
        IplImage *copy_img = cvCreateImage(cvGetSize(image), image->depth, image->nChannels);
        cvCopy(image, copy_img);
        if(centers_mask[i]==false){
            for(size_t j=0;j<similar[i].size();j++){
                if(centers_mask[similar[i][j]]==false)
                    centers_idx.push_back(similar[i][j]);
            }
            for(size_t k=0;k<centers_idx.size();k++){
                if(centers_mask[centers_idx[k]]==false){
                    cvCircle(copy_img, CvPoint(centers[centers_idx[k]][3], centers[centers_idx[k]][4]), 4, CV_RGB(0,0,0), CV_FILLED);
                    centers_mask[centers_idx[k]] = true;
//                    for(size_t h=0;h<similar[centers_idx[k]].size();h++){
//                        if(centers_mask[similar[centers_idx[k]][h]]==false)
//                            centers_idx.push_back(similar[centers_idx[k]][h]);
//                    }
                }
            }
        }
        cvShowImage("mask", copy_img);
        cvWaitKey();
    }
//    for(size_t i=0;i<similar.size();i++){
//        std::cout<<"("<<i<<")->";
...
//        cvCopy(image, copy_img);
//        for(size_t j=0;j<similar[i].size();j++){
//            std::cout<<centers[similar[i][j]][3]<<"  "<<centers[similar[i][j]][4]<<std::endl;
//            cvCircle(copy_img, CvPoint(centers[similar[i][j]][3], centers[similar[i][j]][4]), 2, CV_RGB(0,0,0), CV_FILLED);
//        }
//        cvShowImage("mask", copy_img);
//        cvWaitKey();
////        std::cout<<std::endl;
 ...
 * Output: -
 */
void Slic::display_new_contours(IplImage *image, CvScalar colour) {
    const int dx8[8] = {-1, -1,  0,  1, 1, 1, 0, -1};
	const int dy8[8] = { 0, -1, -1, -1, 0, 1, 1,  1};
    ...
    for (int i = 0; i < image->width; i++) {
        for (int j = 0; j < image->height; j++) {
            ...
            for (int k = 0; k < 8; k++) {
                ...
                if (x >= 0 && x < image->width && y >= 0 && y < image->height) {
                    if (istaken[x][y] == false && new_clusters[i][j] != new_clusters[x][y]) {
                        nr_p += 1;
                    }
                }
            }
